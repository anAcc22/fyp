...
void save_points_to_disk(Generator params, const std::vector<Point> &points) {
...
ClosestPair parallel_grid_decomposition(std::vector<Point> points) {
...
int main() {
    ...
    for (auto n : point_counts) {
        auto stats                      = time_taken_by(parallel_grid_decomposition, generator_for(n, Dimensions{ 2 }));
        auto [closest_pair, time_taken] = stats;
        std::println("Point Count: {:L}, Elapsed Time: {:.3f}s, Closest Pair: {}", n, time_taken.count(), closest_pair);
    }
